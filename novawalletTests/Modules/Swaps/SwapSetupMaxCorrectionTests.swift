...
final class SwapSetupMaxCorrectionTests: XCTestCase {
    ...
    func testStopsRequotingWhenMaxCorrectionAttemptsExhaustedOnSamePath() {
        let context = SwapSetupTestContext.make()
        ...
        context.deliverQuote()
        context.deliverFee(
            submissionAmount: context.payAmountInPlank(2),
            in: context.payChainAsset.chainAssetId
        )

        XCTAssertEqual(context.interactor.quoteRequests.count, 3)

        context.deliverQuote()
        context.deliverFee(
            submissionAmount: context.payAmountInPlank(3),
            in: context.payChainAsset.chainAssetId
        )

        XCTAssertEqual(context.interactor.quoteRequests.count, 3)
        XCTAssertEqual(
            context.interactor.quoteRequests.last?.amount,
            transferable - context.payAmountInPlank(2)
        )
    }
}
