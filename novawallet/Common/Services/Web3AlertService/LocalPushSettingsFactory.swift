protocol LocalPushSettingsFactoryProtocol {
    func createSettings(for wallet: MetaAccountModel,
                        chains: [ChainModel.Id: ChainModel]) -> LocalPushSettings
}

final class LocalPushSettingsFactory {
    func createSettings(for wallet: MetaAccountModel,
                        chains: [ChainModel.Id: ChainModel]) -> LocalPushSettings {
        ...
        let web3Wallet = Web3AlertWallet(
            baseSubstrate: try? wallet.substrateAccountId?.toAddress(using: SNAddressType.genericSubstrate),
