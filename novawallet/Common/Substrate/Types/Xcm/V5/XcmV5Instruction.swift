...
extension XcmV5 {
    ...
    enum Instruction: Codable {
        ...
        case withdrawAsset([XcmV5.Multiasset])
        case depositAsset(DepositAssetValue)
        case clearOrigin
        case reserveAssetDeposited([XcmV5.Multiasset])
        case buyExecution(BuyExecutionValue)
        case depositReserveAsset(DepositReserveAssetValue)
        case receiveTeleportedAsset([XcmV5.Multiasset])
        ...
        init(from decoder: any Decoder) throws {
            ...
            case Self.fieldWithdrawAsset:
                let value = try container.decode([XcmV5.Multiasset].self)
