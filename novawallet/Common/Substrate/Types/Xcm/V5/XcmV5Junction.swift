...
extension XcmV5 {
    ...
    typealias Junctions = Xcm.GenericJunctions<Junction>
}

extension XcmV5.Junctions {
    func appending(components: [XcmV5.Junction]) -> XcmV5.Junctions {
        XcmV5.Junctions(items: items + components)
    }

    func prepending(components: [XcmV5.Junction]) -> XcmV5.Junctions {
        XcmV5.Junctions(items: components + items)
    }

    func lastComponent() -> (XcmV5.Junctions, XcmV5.Junctions) {
        guard let lastJunction = items.last else {
            return (self, XcmV5.Junctions(items: []))
        }

        let remaningItems = Array(items.dropLast())

        return (XcmV5.Junctions(items: remaningItems), XcmV5.Junctions(items: [lastJunction]))
    }
}
