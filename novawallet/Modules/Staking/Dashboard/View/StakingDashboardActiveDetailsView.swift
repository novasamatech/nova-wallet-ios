...
final class StakingDashboardActiveDetailsView: UIView {
    ...
    private func setupStaticLocalization(for locale: Locale, hasEstimatedRewards: Bool) {
        internalStakeView.valueTop.text = R.string(preferredLanguages: locale.rLanguages).localizable.stakingYourStake()

        if hasEstimatedRewards {
            estimatedEarningsView.valueTop.text = R.string(preferredLanguages: locale.rLanguages).localizable.stakingEstimatedEarnings()

            estimatedEarningsView.valueBottom.sView.text = " " + R.string(preferredLanguages: locale.rLanguages).localizable.commonPerYear()
        } else {
