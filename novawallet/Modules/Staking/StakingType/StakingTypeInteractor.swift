...
extension NominationPools {
    enum Lens {
        static let pool = GenericLens<StakingSelectionMethod, NominationPools.SelectedPool?>(
            get: { method in
                ...
                case let .recommendation(recommendation):
                    if let pool = pool, let recommendation = recommendation {
                        return .recommendation(RelaychainStakingRecommendation(
                            staking: .pool(pool),
                            restrictions: recommendation.restrictions,
                            validationFactory: recommendation.validationFactory
                        ))
                    } else {
                        return method
                    }
                case let .manual(choice):
                    if let pool = pool {
                        return .manual(.init(
                            staking: .pool(pool),
