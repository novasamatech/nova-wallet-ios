...
## Spec lenses — applied to `.claude/SPEC.md`

### Lens `contract` — is the document a contract, or a description?

You own §2 Scope, §3 Functional Requirements, §4 Non-Functional Requirements, §5 Contracts,
§10 Verification, §11 Open Questions — and traceability across the whole document.
...
Read `.claude/SPEC.md` and `.claude/PLAN.md`. You are the only lens that sees both.

Build the mapping in both directions and report every hole:

- **Spec → plan.** Walk SPEC §3 (FR), §4 (NFR), §5 (contracts), §8 (edge cases), §9 (migration) item
  by item. For each, find the task that implements it. An item with no task is a finding, and its
...
- **Contract drift.** Compare every signature in PLAN "Contracts" and in the task snippets against
  SPEC §5, character for character. A changed argument label, a dropped `throws`, a swapped optional,
  a return type of `Thing` where the spec said `CompoundOperationWrapper<Thing>` — all findings, and
  all easy to miss because the plan's version reads perfectly well on its own.
...
- **Internal signature consistency.** Every "Produces" block against every "Consumes" that cites it,
  and against the code in the task bodies. The plan format exists to keep these aligned; check that
  it did.
...
Read **only** `.claude/PLAN.md`. Do **not** open `SPEC.md`, and do not go looking for the design
rationale. You are simulating the executor, and the executor has neither. If you need something the
plan does not contain, that absence *is* the finding.
...
- **Placeholders.** `TBD`, `...` inside a code block, an empty function body, a comment standing in
  for logic.
- **Undefined names.** A symbol used in a task that no earlier task and no cited `file:line`
  ...
  job — but you *can* check whether the plan told you where it comes from. It must.
